...
def predict():
		...
		if (prediction == -1):
			return render_template('index.html', label = "You're an amateur! Play more, improve your game to be amongst the top 10.")
		else:
			prediction = model2.predict_proba(parameters.reshape(1,-1))
			label = "You're an amongst the top 10. Rank Probabilities :" + str(prediction)
			return render_template('index.html', label = label)
	
if __name__ == '__main__':
	# load ml model
	model1 = joblib.load('top10_Prediction.pkl')
	model2 = joblib.load('top10_Probabilities.pkl')
	# start api
